...
import { transformDataForCouponNrSemesterGraph } from '@/app/lib/utils';
...
export default function CouponTypes6MonthBreakdownChartByNumber({
    currentPeriod,
    last6Months,
    chartData,
    couponTypes,
    ...
    const transformedData = transformDataForCouponNrSemesterGraph(
        chartData,
        last6Months
    );
    ...
    const accumulatedNumbersByCouponPrefix = transformedDataKeys.reduce(
        (acc, key) => {
    ...
    const top3ByValue = Object.entries(accumulatedNumbersByCouponPrefix)
        .toSorted((a, b) => b[1] - a[1])
        .slice(0, 3);
    const bottom3ByValue = Object.entries(accumulatedNumbersByCouponPrefix)
    ...
    return (
        <Card>
            <CardHeader>
                <CardTitle>{currentPeriod} (nr of redeems)</CardTitle>
                <CardDescription>
                    Coupon statistics by number of redeems for the last 6
                    months.
                </CardDescription>
            </CardHeader>
            <CardContent>
            ...
            <CardFooter className='flex-col items-start gap-2 text-sm'>
                ...
                <div className='grid md:grid-cols-2 lg:grid-cols-4 gap-4 mt-2 w-full'>
                    <Card className='col-span-2'>
                        <CardHeader>
                            <CardTitle className='text-base'>
                                Top 3 by Nr of Redeems
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            {top3ByValue.map(([coupon_prefix, coupon_nr]) => (
                                <div
                                    key={coupon_prefix}
                    ...
                    <Card className='ml-2 col-span-2'>
                        <CardHeader>
                            <CardTitle className='text-base'>
                                Bottom 3 by Nr of Redeems
                            </CardTitle>
                        </CardHeader>
                        <CardContent>
                            {bottom3ByValue.map(
                                ([coupon_prefix, coupon_nr], index) => (
                                    <div
